...
bash -n "$repo_root/infra/installer/install.sh" "$repo_root/infra/installer/ship" "$repo_root/scripts/build-production-images.sh" "$repo_root/scripts/test-image.sh" "$repo_root/scripts/package-release.sh"
...
first_master_key="$(awk -F= '$1 == "SHIP_MASTER_KEY" {print $2}' "$install_root/.env")"
bash "$repo_root/infra/installer/install.sh" --hostname ship.example.com >"$temporary/second.log"
second_master_key="$(awk -F= '$1 == "SHIP_MASTER_KEY" {print $2}' "$install_root/.env")"
bash "$repo_root/infra/installer/install.sh" >"$temporary/third.log"

[[ "$first_master_key" == "$second_master_key" ]]
